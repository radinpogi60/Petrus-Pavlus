	...
	if (isset($_SESSION['username'])) {
		 ...
		 $date_now = date('m/d/Y');
		 $time_now = date('h:i a');
		 $sql_id = mysqli_query($conn, "SELECT * FROM users WHERE status=1");
	...
	if (isset($_GET['id'])) {
		$id_absent = $_GET['id'];
		$present_sql = mysqli_query($conn, "UPDATE users SET status=0 WHERE user_id='$id_absent'");
		header("location:attendance_out");
	}
		...
		<div class="header-attendance">
			...
			<center>
				<ul class="list-inline">
					<li>
						<a href="attendance" class="btn btn-success"><i class="fa fa-sign-in"></i> In</a>
					</li>
					<li>
						<button class="btn btn-danger" disabled><i class="fa fa-sign-out"></i> Out</button>
					</li>
				</ul>
			</center>
			<!-- <hr> -->
				...
				<?php	
					 while ($fetch_user = mysqli_fetch_assoc($sql_id)	) {
						...
						<tr>
							...
							<td>
								<center>
									<a href="attendance_out?id=<?php echo $db_user_id;?>" class="btn btn-danger" onclick="return confirm('Sign out user <?php echo $db_user_name;?>?')"><i class="fa fa-fw fa-sign-out"></i></a>
